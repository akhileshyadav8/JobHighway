            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {displayedCompanies.map((company) => (
                  <Card key={company.id} className="hover:shadow-lg transition-all border-slate-200 hover:-translate-y-1">
                    <CardContent className="p-6">
                      <div className="flex items-start gap-4 mb-4">
                        <div className="w-14 h-14 rounded-2xl bg-teal-100 flex items-center justify-center text-teal-700 font-bold text-2xl flex-shrink-0 shadow-inner">
                        ...
                        <div>
                          ...
                          <div className="text-xs font-semibold text-teal-600 mt-1">{company.industry}</div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between border-t border-slate-100 pt-4">
                        <Link 
                          ...
                          className="inline-flex items-center text-teal-600 font-semibold text-xs hover:underline"
                        >
              ...
              {totalPages > 1 && (
                <div className="mt-12 flex flex-col md:flex-row items-center justify-between gap-4 p-4 rounded-2xl bg-white border border-slate-200 shadow-sm">
                  <div className="text-xs sm:text-sm text-slate-500 font-medium">
                    Showing <strong className="text-slate-900 ">{((currentPage - 1) * COMPANIES_PER_PAGE) + 1}</strong>–<strong className="text-slate-900 ">{Math.min(currentPage * COMPANIES_PER_PAGE, filtered.length)}</strong> of <strong className="text-slate-900 ">{filtered.length}</strong> companies (Page {currentPage} of {totalPages})
                  </div>
                        ...
                        <Button
                          ...
                          className={`w-8 h-8 p-0 rounded-xl text-xs font-bold cursor-pointer ${
                            p === currentPage
                              ...
                              : "text-slate-700 hover:border-teal-500 hover:text-teal-600"
                          }`}
                        >
                          {p}
                        </Button>
                  ...
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    <input
                      ...
                      className="w-14 px-2 py-1 border border-slate-200 rounded-lg bg-slate-50 text-slate-900 text-center outline-none focus:border-teal-500"
                    />
                    <Button
                      variant="outline"
                      size="sm"
