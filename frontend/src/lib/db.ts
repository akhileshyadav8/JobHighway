...
export async function getLiveJobsFromDb(limit: number = 300): Promise<Job[] | null> {
  const p = getPool();
  if (!p) return null;
  ...
  try {
    ...
    return res.rows.map(row => ({
      ...
      salary_basis: row.salary_basis || null,
      is_salary_estimated: row.is_salary_estimated || false,
